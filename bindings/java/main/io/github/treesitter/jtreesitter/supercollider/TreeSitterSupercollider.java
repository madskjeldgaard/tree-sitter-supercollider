package io.github.treesitter.jtreesitter.supercollider;

import java.lang.foreign.*;

public final class TreeSitterSupercollider {
    ...
    private static final TreeSitterSupercollider INSTANCE = new TreeSitterSupercollider();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterSupercollider() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_supercollider");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-supercollider");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-supercollider");
